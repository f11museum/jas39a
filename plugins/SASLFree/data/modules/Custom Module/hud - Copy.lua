...
function process_targets()
	...
	local acf_x, acf_y, acf_z = get(dr_acf_x), get(dr_acf_y), get(dr_acf_z)
	...
	for i = 2, 20 do
		if mp_thro[i] ~= nil then
			...
			target[i].brg, target[i].elev, target[i].dist = getangles(get(dr_multiplayer_x[i-1]), get(dr_multiplayer_y[i-1]), get(dr_multiplayer_z[i-1]), acf_x, acf_y, acf_z)
...
-- print("calcing" )
	...
	-- ref speeds
	weight_speed = (sim_total_weight_kg - 10000) / 6000 * 30
	landing_speed = 115 + weight_speed
	min_speed = 90 + weight_speed
...
-- Heading
			...
			-- Speed
			drawRectangle(-250, -200, 60, 120, black)
			setClipArea(-250, -200, 80, 120)
			drawTexturePart(symbols, -200, -200, 11, 119, 42, 175 + (sim_airspeed/10) % 1 * 24, 11, 119, hudcol)
			if sim_airspeed > 20 then 
				drawTexturePart(symbols, -210, -150, 15, 15, 190, 8, 15, 15, hudcol)
				drawTexturePart(symbols, -225, -154, 13, 20, 7, 126 + sim_airspeed % 10 * 16, 11, 16, hudcol)
				if sim_airspeed > 9 then drawTexturePart(symbols, -238, -154, 13, 20, 7, 126 + math.floor(sim_airspeed/10) % 10 * 16, 11, 16, hudcol) end
				if sim_airspeed > 99 then drawTexturePart(symbols, -251, -154, 13, 20, 7, 126 + math.floor(sim_airspeed/100) % 100 * 16, 11, 16, hudcol) end
			else
			...
			if sim_alt >= 3000 then 
				drawTexturePart(symbols, 200, -200, 11, 119, 69, 172 + (sim_alt/200) % 1 * 52, 11, 119, hudcol) --tape
				...
				drawTexturePart(symbols, 200, -200, 11, 119, 69, 172 + (sim_alt/100) % 1 * 52, 11, 119, hudcol) --tape
				-- drawTexturePart(symbols, 185, -150, 15, 15, 190, 8, 15, 15, hudcol)
				for i = -1, 2, 1 do
					alt2 = sim_alt + 100 * i
					-- if sim_alt - alt2 < 120 and sim_alt - alt2 > -320 then 
						if alt2 >= 1000 then drawTexturePart(symbols, 215, -154 - (alt2/100) % 1 * 52 + 52 * i, 13, 20, 7, 126 + math.floor(alt2/1000 + 500) % 10 * 16, 11, 16, hudcol) end
						drawTexturePart(symbols, 226, -154 - (alt2/100) % 1 * 52 + 52 * i, 11, 16, 7, 126 + math.floor(alt2/100) % 10 * 16, 11, 16, hudcol)
						drawTexturePart(symbols, 237, -154 - (alt2/100) % 1 * 52 + 52 * i, 11, 16, 7, 126 , 11, 16, hudcol)
						drawTexturePart(symbols, 248, -154 - (alt2/100) % 1 * 52 + 52 * i, 11, 16, 7, 126 , 11, 16, hudcol)
						-- end
					end
				end
			resetClipArea()
		...
		setTranslateTransform(target[i].screen_x, target[i].screen_y)
			if not target[i].clip then
				...
				if target[i].active then 
					...
					sasl.gl.drawLinePattern (0, 0, 0, -20 - math.abs(to_brg(target[i].brg - sim_true_heading))/2, false,  hudcol )
					end
	...
	-- print("sim_running " .. sim_running)
		...
		saveGraphicsContext()
			...
			for i = -2, 2 do
				...
				drawWideLine(-tempheading, 0, -tempheading, 5, 2, hudcol)
				end
			...
			saveGraphicsContext()
				...
				if hud_mode == "to" and sim_radalt < 5 then 
					setTranslateTransform(0, -sim_acf_pitch * screen_vdegrees + (-2 + clamp((sim_airspeed - rotation_speed) / 3, 0, 13)) * screen_vdegrees)
					drawTexturePart(symbols, -27, -17, 54, 35, 0, 0, 54, 36, hudcol)
				else
					setTranslateTransform(-hvector * screen_hdegrees, clamp(-vvector, -14, 5) * screen_vdegrees)
			...
			-- Bore sight
			sasl.gl.drawWideLine(-10, 0, 10, 0, 2, hudcol)
			sasl.gl.drawWideLine(0, 0, 0, 10, 2, hudcol)
		...
		setBlendFunction(BLEND_SOURCE_COLOR, BLEND_DESTINATION_ALPHA)
		drawTexture(rendertarget, -512, -512, 1024, 1024, {1, 0.75, 1, 1.0})
